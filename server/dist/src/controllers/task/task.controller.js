...
import { broadcastTaskStatusChange, broadcastTaskAssigned, broadcastCommentAdded, broadcastTaskDeleted, broadcastTaskUpdated, } from '../../services/socket.service.js';
...
const updateTask = asyncHandler(async (req, res) => {
    ...
    broadcastTaskUpdated(req, req.project.slug, serializeTask(updatedTask, req.project?.owner_id), req.task);
    return successResponse(res, serializeTask(updatedTask, req.project?.owner_id), 'Task updated successfully');
});
...
const changeTaskStatus = asyncHandler(async (req, res) => {
    ...
    // Invalidate Redis stats cache
    await invalidateProjectStats(req.project.slug);
    broadcastTaskStatusChange(req, req.project.slug, serializeTask(updatedTask, req.project?.owner_id), req.task);
    return successResponse(res, serializeTask(updatedTask, req.project?.owner_id), 'Task status updated successfully');
});
...
const assignTask = asyncHandler(async (req, res) => {
    ...
    broadcastTaskAssigned(req, req.project.slug, serializeTask(updatedTask, req.project?.owner_id), req.task);
    return successResponse(res, serializeTask(updatedTask, req.project?.owner_id), 'Task assignment updated successfully');
});
...
const reorderTasks = asyncHandler(async (req, res) => {
    ...
    try {
        await prisma.$transaction(tasks.map((t) => prisma.tasks.update({
            where: { id: t.id },
...
const createTaskComment = asyncHandler(async (req, res) => {
    ...
    broadcastCommentAdded(req, req.project.slug, serializeComment({ ...comment, user: comment.users }));
    return successResponse(res, serializeComment({ ...comment, user: comment.users }), 'Comment added successfully', 201);
});
...
const showTask = asyncHandler(async (req, res) => {
    return successResponse(res, serializeTask(req.task, req.project?.owner_id), 'Task details retrieved successfully');
});
export { updateTask, changeTaskStatus, assignTask, reorderTasks, deleteTask, listTaskComments, createTaskComment, listTaskActivities, showTask, };
