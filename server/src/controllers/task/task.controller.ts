...
const updateTask = asyncHandler(async (req, res) => {
  ...
  await invalidateProjectStats((req as any).project.slug);

  broadcastTaskUpdated(
    req,
    (req as any).project.slug,
    serializeTask(updatedTask, (req as any).project?.owner_id),
    req.task
  );
...
const reorderTasks = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return errorResponse(res, (error as any).message, 400);
  }
...
const deleteTask = asyncHandler(async (req, res) => {
  ...
  broadcastTaskDeleted(req, (req as any).project.slug, taskId);
...
const listTaskComments = asyncHandler(async (req, res) => {
  ...
  serializedComments.forEach((c: any) => {
    commentMap[c.id] = c;
  });
...
const showTask = asyncHandler(async (req, res) => {
  return successResponse(
    res,
    serializeTask(req.task, (req as any).project?.owner_id),
