...
const projectCreateSchema = z.object({
  ...
  start_date: z
    .preprocess(parseDate, z.date({ message: 'Invalid start date' }))
    .default(() => new Date()),
  ...
  end_date: z
    .preprocess(
      ...
      z.date({ message: 'Invalid end date' }).nullable()
    )
    .optional(),
...
const validateCreateProject = (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const msg = errors ? errors.map((e: any) => e.message).join(', ') : 'Invalid project data';
