...
const positiveInteger = (field) => z.preprocess(parseInteger, z.number().int().positive(`${field} must be a positive integer`));
const taskHours = (field) => z.preprocess(parseDecimal, z
    ...
    .refine((value) => /^\d{1,4}(\.\d)?$/.test(String(value)), `${field} must be a valid DECIMAL(5,1)`));
...
const taskReorderSchema = z.array(z.object({
    id: z.preprocess((val) => (val === undefined ? undefined : Number(val)), z.number().int().positive('Task ID must be a positive integer')),
    sort_order: z.preprocess((val) => (val === undefined ? undefined : Number(val)), z.number().int('Sort order must be an integer')),
}));
...
export { validateCreateTask, validateUpdateTask, validateChangeTaskStatus, validateAssignTask, validateReorderTasks, taskCreateSchema, taskUpdateSchema, taskStatusSchema, taskAssignSchema, taskReorderSchema, };
