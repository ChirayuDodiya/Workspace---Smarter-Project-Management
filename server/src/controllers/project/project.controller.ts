...
const listProjects = asyncHandler(async (req, res) => {
  ...
  const statsMap: Record<number, { total: number, completed: number }> = {};
  ...
  const serialized = projects.map((project: any) => {
    const stats = statsMap[project.id] || { total: 0, completed: 0 };
    const serializedProject = serializeProject(project);
    (serializedProject as any).task_count = stats.total;
    (serializedProject as any).completed_tasks = stats.completed;
...
const listTasks = asyncHandler(async (req, res) => {
  ...
  const serialized = tasks.map((task: any) => serializeTask(task, project.owner_id));
...
const teamMembers = asyncHandler(async (req, res) => {
  ...
  const usersList = members.map((m: any) => m.users).filter(Boolean);
  const serializedUsers = usersList.map((user: any) => serializeUser(user));
