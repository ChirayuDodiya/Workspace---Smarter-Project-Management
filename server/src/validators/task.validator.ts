...
const taskCreateSchema = z.object({
  ...
  due_date: z.preprocess(parseDate, z.date({ message: 'Invalid due date' })).optional(),
...
const validateCreateTask = (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const msg = errors ? errors.map((e: any) => e.message).join(', ') : 'Invalid task data';
...
const validateChangeTaskStatus = (req: Request, res: Response, next: NextFunction) => {
  try {
    req.body = taskStatusSchema.parse(req.body);
    return next();
  } catch (err: any) {
    const errors = err?.issues || err?.errors;
    const msg = errors ? errors.map((e: any) => e.message).join(', ') : 'Invalid task status data';
    return errorResponse(res, msg, 400);
  }
};
...
const validateAssignTask = (req: Request, res: Response, next: NextFunction) => {
  try {
    req.body = taskAssignSchema.parse(req.body);
    return next();
  } catch (err: any) {
    const errors = err?.issues || err?.errors;
    const msg = errors ? errors.map((e: any) => e.message).join(', ') : 'Invalid assignment data';
    return errorResponse(res, msg, 400);
  }
};
...
const validateReorderTasks = (req: Request, res: Response, next: NextFunction) => {
  try {
    req.body = taskReorderSchema.parse(req.body);
    return next();
  } catch (err: any) {
    const errors = err?.issues || err?.errors;
    const msg = errors ? errors.map((e: any) => e.message).join(', ') : 'Invalid task reorder data';
    return errorResponse(res, msg, 400);
  }
};
