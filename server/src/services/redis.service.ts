...
export const getCachedStats = async (slug: string) => {
...
export const setCachedStats = async (slug: string, data: any) => {
  if (!redis) return;
  try {
...
export const invalidateProjectStats = async (slug: string) => {
  if (!redis) return;
  try {
