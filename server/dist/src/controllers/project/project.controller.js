...
import { getCachedStats, setCachedStats, invalidateProjectStats, } from '../../services/redis.service.js';
// GET: /api/v1/projects — List with filtering (status, owner), sorting, and pagination
const listProjects = asyncHandler(async (req, res) => {
    const { status, owner, search, page = 1, per_page = 20, sortBy = 'created_at', order = 'desc', } = req.query;
    ...
    const [total, projects] = await Promise.all([
        ...
        prisma.projects.findPaginated(where, orderField, orderDirection, (pageNumber - 1) * pageSize, pageSize),
    ]);
...
const updateProject = asyncHandler(async (req, res) => {
    ...
    return successResponse(res, serializeProject(updatedProject), 'Project updated successfully', 200);
});
...
const listTasks = asyncHandler(async (req, res) => {
    ...
    const { status, priority, assigned_to, search, page = 1, per_page = 20, sortBy = 'sort_order', order = 'asc', } = req.query;
    const pageNumber = Math.max(Number(page) || 1, 1);
    const pageSize = Math.min(Math.max(Number(per_page) || 20, 1), 100);
    ...
    if (priority)
        whereFilters.priority = priority;
    if (assigned_to)
        whereFilters.assigned_to = assigned_to;
    if (search) {
        whereFilters.title = {
    ...
    const [total, tasks] = await Promise.all([
        ...
        prisma.tasks.findForProject(project.id, whereFilters, orderField, orderDirection, (pageNumber - 1) * pageSize, pageSize),
    ]);
...
const createTask = asyncHandler(async (req, res) => {
    ...
    return successResponse(res, serializeTask(task, project.owner_id), 'Task created successfully', 201);
});
...
export { listProjects, createProject, showProject, updateProject, deleteProject, projectStats, listTasks, createTask, teamMembers, listManagers, addTeamMember, removeTeamMember, exportProjectTasks, };
